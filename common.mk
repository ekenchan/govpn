...
LDFLAGS = -X cypherpunks.ru/govpn.Version=$(VERSION) -s -w
PREFIX ?= /usr/local
...
govpn-client:
	GOPATH=$(GOPATH) CGO_ENABLED=0 go build -ldflags "$(LDFLAGS)" cypherpunks.ru/govpn/cmd/govpn-client

govpn-server:
	GOPATH=$(GOPATH) CGO_ENABLED=0 go build -ldflags "$(LDFLAGS)" cypherpunks.ru/govpn/cmd/govpn-server

govpn-verifier:
	GOPATH=$(GOPATH) CGO_ENABLED=0 go build -ldflags "$(LDFLAGS)" cypherpunks.ru/govpn/cmd/govpn-verifier
